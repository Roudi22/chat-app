      ...
      <ScrollArea className='gap-2 px-2 group-[[data-collapsed=true]]:justify-center group-[[data-collapsed=true]]:px-2'>
          {USERS.map((user, idx) => (
            isCollapsed ? (
              ...
              <Button className={cn("w-full justify-start gap-4 my-1", selectedUser.email === user.email && "dark:bg-muted dark:text-white dark:hover:bg-muted dark:hover:text-white shrink")} variant={"grey"} size="xl" key={idx}>
                <Avatar className='my-1 flex justify-center items-center'>
                        <AvatarImage src={user.image || "../../public/avatars/user1.png"} alt={user.name} 
                        className='w-10 h-10'
                        />
          ...
          <div className='flex justify-between items-center gap-2 md:px-6 py-2'>
            {!isCollapsed && (
              <div className='hidden md:flex gap-2 items-center'>
                <Avatar>
                  <AvatarImage src={"/user-placeholder.png"} alt={selectedUser.name} referrerPolicy='no-referrer' className='w-9 h-9 border-2 border-white rounded-full'/>
